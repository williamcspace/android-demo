...
public class CreateOrderJob extends Job {
    public final String TAG = LogUtils.makeLogTag(CreateOrderJob.class);

    private JsonObject mBody;

    public CreateOrderJob(JsonObject jo) {
        // requireNetwork() 需要联网,
    ...
    public void onRun() throws Throwable {
        ...
        RestServiceProvider.getTangxlApi().createOrder(mBody, new Callback<JsonObject>() {
            @Override
            public void success(final JsonObject jsonObject, Response response) {
                LogUtils.d(TAG, jsonObject.toString());
                EventBusProvider.getInstance().post(new OnCreateOrderFinish(false, "Fetch posts success", jsonObject));
            }

            @Override
            public void failure(RetrofitError error) {
                LogUtils.d(TAG, "Fetch Posts failure: " + error);
                EventBusProvider.getInstance().post(new OnCreateOrderFinish(true, "Fetch Posts failure:" + error));
            }
        });
    }
    ...
    protected void onCancel() {
        ...
        EventBusProvider.getInstance().post(new OnCreateOrderFinish(true, "job Cancelled"));
    }
}