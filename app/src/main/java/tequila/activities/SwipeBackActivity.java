...
import com.tangxinli.android.tequila.layouts.SwipeBackLayout;
 ...
 */
public class SwipeBackActivity extends BaseActivity implements SwipeActivityManager.SwipeListener, SwipeBackLayout.OnSwipeGestureDelegate {
    ...
    public void onScrollParent(float scrollPercent) {
        LogUtils.v(TAG, "on swipe " + scrollPercent + " ,duration " + Long.valueOf(240L));
    ...
    public void notifySettle(boolean open, int speed) {
        LogUtils.v(TAG, "on settle " + open + ", speed " + speed);
        View decorView = getWindow().getDecorView();
        if ((decorView instanceof ViewGroup) && (((ViewGroup) decorView).getChildCount() > 0)) {
            decorView = ((ViewGroup) decorView).getChildAt(0);
        }
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        if (isSupperSwipe() && event.getKeyCode() == KeyEvent.KEYCODE_BACK && mSwipeBackLayout.isSwipeBacking()) {
            LogUtils.d(TAG, "IS SwipeBack ING, ignore KeyBack Event");
            return true;
        }
